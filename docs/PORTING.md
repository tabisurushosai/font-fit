...
- 保存処理は `src/storage` のアダプタ越しに行います。アプリ版では `createStorageAdapter(port)` に移植先の保存先を渡すか、同じ `FontFitStorage` IF を実装します。
...
- `FontFitStoragePort` は `get(keys)` / `set(items)` だけを持つ低レベル IF です。iOS / Android では Keychain、SharedPreferences、SQLite などのローカル保存先をこの形に合わせます。
- `FontFitStorageValueMap` は保存キーごとの値の型、`FontFitStorageItems` はその `Partial` です。移植先の port はこの型どおりに値を返し、未保存のキーは `undefined` のまま扱います。
- `createStorageAdapter(port)` は `FontFitStoragePort` から `FontFitStorage` を作るプラットフォーム非依存の実装です。Chrome 拡張では `src/storage/chromeStorage.ts` の `createChromeStorageAdapter()` が `chrome.storage.local` を注入します。
- 既存の `FontFitStorageArea` / `StorageItems` / `StorageKey` 系の型名は互換 alias として残しています。新規の移植コードでは `FontFitStoragePort` / `FontFitStorageItems` / `FontFitStorageKey` 系を使ってください。
...
- `src/core` へ新しい共通処理を追加するときは、引数と戻り値を plain object / primitive に保ち、Chrome 型、DOM 型、保存 port 型を持ち込まないでください。
- Chrome 拡張以外の entrypoint では、`src/storage` から `createStorageAdapter` と型だけを import し、移植先専用の保存 port をアプリ側で注入してください。
